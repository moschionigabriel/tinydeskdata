(function() {
	var tinyDeskData = {
		...
		if (obj.source.where == 'drive') {
			...
			let file_name = obj.source.config.file_name
		...
		for(model of obj.models) {
			let run_query = `CREATE OR REPLACE ${model.materialized.toUpperCase()} ${obj.config.credentials.project_name}.${model.schema_name}.${model.name} AS (${model.compiled_code})`
